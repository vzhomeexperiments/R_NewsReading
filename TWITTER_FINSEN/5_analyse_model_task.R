...
# file names
filesToAnalyse <-list.files("C:/Users/fxtrams/Documents/000_TradingRepo/R_NewsReading/TWITTER_FINSEN/Logs/", pattern="    _",
                            full.names=TRUE)
...
for (FILE in filesToAnalyse) {
  ...
  tryCatch({
    ...
    DateTime <- str_remove(FILE, pattern = "   _")
    # remove the right part .csv
    DateTime <- DateTime %>% str_remove(pattern = '   ')
    # make this as date
    DateTime <- DateTime %>% ymd() 
    #class(DateTime)
    
    # read content of the file
    prediction <- read_csv(   ) %>% select(   )
...
if(sum(   $    ) > 0) {print("We can trust!")} else {print("We can not trust! (yet?)")
                                                                     sum(  $   )}
